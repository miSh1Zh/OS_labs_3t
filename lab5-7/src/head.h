...
class TNode{
    ...
    public:
        ...
        TNode(int idElem, int parentP, int parID): id(idElem), parentPort(parentP), 
        parentId(parID), leftId(-2), rightId(-2),
        ...
        std::string exec(std::string str, std::map<std::string, int>& wiki_node){
            ...
            if(command >> val){
                ...
                if(wiki_node.find(key) != wiki_node.end()){
                    answer += ": " + std::to_string(wiki_node[key]);
                }else{
